...
protected:
    void SetUp() override {
        // Инициализация криптографии (выбор реализации)
        crypto::sha256_init();
    }
};
...
TEST_F(SHA256Test, EmptyMessage) {
    ...
    auto hash = crypto::sha256(empty.data(), 0);
    
    // Ожидаемый хеш пустого сообщения
    constexpr core::Hash256 expected = {{
...
TEST_F(SHA256Test, SimpleMessage) {
    ...
    auto hash = crypto::sha256(reinterpret_cast<const uint8_t*>(msg.data()), msg.size());
...
TEST_F(SHA256Test, DoubleSHA256) {
    ...
    auto hash = crypto::sha256d(reinterpret_cast<const uint8_t*>(msg.data()), msg.size());
...
TEST_F(SHA256Test, Midstate) {
    ...
    auto midstate = crypto::sha256_midstate(data.data(), data.size());
    
    // Midstate должен быть 32 байта
...
/**
 * @brief Тест: финализация с midstate
 * 
 * Проверяет, что sha256_finalize с midstate даёт тот же результат,
 * что и полное хеширование.
 */
TEST_F(SHA256Test, MidstateFinalize) {
    // 80 байт данных (как заголовок блока Bitcoin)
    ...
    // Полный хеш
    auto full_hash = crypto::sha256(data.data(), data.size());
    
    // Хеш через midstate
    auto midstate = crypto::sha256_midstate(data.data(), 64);
    auto partial_hash = crypto::sha256_finalize(midstate, data.data() + 64, 16);
    
    EXPECT_EQ(full_hash, partial_hash) << "Хеш через midstate должен совпадать с полным хешем";
}
...
TEST_F(SHA256Test, Performance) {
    ...
    for (size_t i = 0; i < iterations; ++i) {
        [[maybe_unused]] auto hash = crypto::sha256(data.data(), data.size());
    }
    
    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
    
    double ns_per_hash = static_cast<double>(duration.count()) / iterations;
...
TEST_F(SHA256Test, BitcoinBlockHeader) {
    ...
    auto hash = crypto::sha256d(header.data(), header.size());
